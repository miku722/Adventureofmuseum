...
interface CombinedScenes1to5Props {
  onComplete: () => void;
  playerName: string;
  gameState: GameState;
...
export function CombinedScenes1to5({
  onComplete,
  playerName,
  gameState,
  onUpdateGameState,
}: CombinedScenes1to5Props) {
...
这光芒如此强烈，摄像头传感器都过曝了...`,
      initialMessage: `${playerName}！紧急情况！主展厅A区监控刚才中断了3秒，现在恢复了，但是...那尊青铜鼎正在发光！电磁读数异常，这不科学！`,
      minMessages: 2,
    },
    {
      // 场景3：不属于此世的光
...
它们在觉醒！`,
      initialMessage: `${playerName}，天哪！不只是青铜鼎了！唐三彩马俑的眼睛在发光，汉代玉璧正在...它在悬浮！能量读数完全爆表！`,
      minMessages: 3,
    },
    {
      // 场景4：文明的断裂
...
这是你即将被吞没的最后一刻！`,
  ...
  const buildSystemPrompt = () => {
    ...
    // 添加当前场景的特定上下文
    const sceneContext = SCENE_CONTEXTS[currentSceneConfig.sceneKey];
